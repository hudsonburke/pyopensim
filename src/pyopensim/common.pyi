...
if TYPE_CHECKING:
    # Import SimTK types from simbody for type checking only  
    from pyopensim.simbody import (
        Mat33,
        Matrix,
        State,
        UnitVec3,
...
# SimTK template instantiations - map C++ template names to Python concrete classes
Mat = Mat33  # SimTK::Mat<3,3>
Matrix_ = Matrix  # SimTK::Matrix_<Real>
UnitVec = UnitVec3  # SimTK::UnitVec<Real,1>
Vector_ = Vector  # SimTK::Vector_<Real>
Vec = Vec3  # SimTK::Vec<3>

# OpenSim types not fully exposed to Python
Object = Any  # OpenSim::Object base class (not exposed)
AbstractValue = Any  # SimTK::AbstractValue (not exposed)
CacheEntryIndex = Any  # SimTK::CacheEntryIndex (not exposed)
MultibodySystem = Any  # SimTK::MultibodySystem (not exposed)
Array = Any  # OpenSim::Array generic (multiple concrete types below)
ArrayPtrs = Any  # OpenSim::ArrayPtrs forward reference
ComponentList = Any  # Forward reference (defined below)
ComponentListIterator = Any  # Forward reference (defined below)
DataTable_ = Any  # OpenSim::DataTable template (multiple concrete types below)
Input = Any  # Forward reference - OpenSim Input classes defined below
Output = Any  # Forward reference - OpenSim Output classes defined below
Set = Any  # Forward reference - OpenSim Set class defined below
Stage = Any  # SimTK::Stage enum (not exposed)
String = str  # C++ std::string maps to Python str
STOFileAdapter_ = Any  # Forward reference - STOFileAdapter template types defined below
AbstractDataTable = Any  # Forward reference (defined below)
DataAdapter = Any  # Forward reference (defined below)
TableSource_ = Any  # Forward reference - TableSource template types defined below
TimeSeriesTable_ = Any  # Forward reference - TimeSeriesTable template types defined below
...
class LogSink:
    def __init__(self, self2:  Any) -> None: ...
...
class Storage:
    ...
    @overload
    def getDataColumn(self, columnName: str, rData: Any, columnName2:  str, data: Array, startTime: float) -> int: ...
    def getDebugLevel(self) -> int: ...
    def getDefaultInstanceOfType(self, concreteClassName: str) -> Any: ...
    def getDescription(self) -> str: ...
    def getDocumentFileName(self) -> str: ...
